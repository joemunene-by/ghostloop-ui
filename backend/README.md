...
2. **New Project**, then **Deploy from GitHub repo**, then pick `joemunene-by/ghostloop-ui`.
3. After the service is created, open **Settings**, then **Service**:
   - **Root Directory:** `backend` (this is the key step. Without it Railway tries to build the whole repo and fails because the root is a Next.js project).
4. **Variables**, then **Add**:
...
6. Once green, open **Settings**, then **Networking**, then **Generate Domain** to get an HTTPS URL like `ghostloop-backend.up.railway.app`.

## Wire the UI to this backend

In Vercel, open the ghostloop-ui project, then **Settings**, then **Environment Variables**:
